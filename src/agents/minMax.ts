...
export async function minMaxWrapper(state: State, depth: number, maxPlayer: Player|null=null): Promise<Notation> {
  if (depth <= 0) console.warn("minmax with depth <= 0");
...
export function minMax({ state, alpha, beta, maxPlayer, depth, maximizing }: minMaxArgs): number {
  ...
  // if (state.automaticPlayoutPossible) {
    ...
    // return state.automaticPlayout() === maxPlayer ? Number.POSITIVE_INFINITY : Number.NEGATIVE_INFINITY;
  // }
  if (depth <= 0) {
